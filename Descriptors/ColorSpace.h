...
public:
	...
	// accessor methods
	virtual bool GetColorReferenceFlag(void) const;
	virtual unsigned long SetColorReferenceFlag(bool colorReferenceFlag);
	virtual ColorSpaceType GetDescriptionColorSpace(void) const;
	virtual unsigned long SetDescriptionColorSpace(ColorSpaceType);
	virtual const float* GetColorSpaceMatrix() const;
	virtual float GetMatrixElement(int xindex,int yindex) const;
