...
Frame::Frame(int width, int height, bool imgFlag, bool grayFlag, bool maskFlag) :
	width(width), height(height), image(0), gray(0), mask(0) {
	this->allocate(width, height, imgFlag, grayFlag, maskFlag);
}

Frame::Frame(Mat& src, bool imgFlag, bool grayFlag, bool maskFlag) :
		width(src.cols), height(src.rows), image(0), gray(0), mask(0) {
	this->allocate(src.cols, src.rows, imgFlag, grayFlag, maskFlag);
	this->setImage(src);
}

void Frame::allocate(int width, int height, bool imgFlag, bool grayFlag,
		bool maskFlag) {
	/// 3-channel color image
	if (imgFlag) {
		if (!image)
			image = initVop(width, height, NONE_TYPE, UCHAR_TYPE, UCHAR_TYPE,
					UCHAR_TYPE);
		...
		if (!image) {
			std::cerr
					<< "Error: Frame :: allocate() - image - memory allocation failed!"
					<< std::endl;
			return;
		}
	...
	if (grayFlag) {
		if (!gray)
			gray = initVop(width, height, NONE_TYPE, UCHAR_TYPE, NONE_TYPE,
					NONE_TYPE);
		...
		if (!gray) {
			std::cerr
					<< "Error: Frame :: allocate() - gray - memory allocation failed!"
					<< std::endl;
			return;
		}
	}
	...
	if (maskFlag) {
		if (!mask)
			mask = initVop(width, height, UCHAR_TYPE, NONE_TYPE, NONE_TYPE,
					NONE_TYPE);
		...
		if (!mask) {
			std::cerr
					<< "Error: Frame :: allocate() - mask - memory allocation failed!"
					<< std::endl;
			return;
		}
	}
...
void Frame::resize(int width, int height) {
	...
	if (this->image) {
		freevop(this->image);
		this->image = initVop(width, height, NONE_TYPE, UCHAR_TYPE, UCHAR_TYPE,
				UCHAR_TYPE);
	}

	if (this->gray) {
		freevop(this->gray);
		this->gray = initVop(width, height, NONE_TYPE, UCHAR_TYPE, NONE_TYPE,
				NONE_TYPE);
	}
